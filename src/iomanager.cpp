...
uint32_t IoManager::SetEvent(int fd, uint32_t ev, Coroutine::Ptr co) {
    ...
    if (epoll_ctl(epoll_fd_, op, fd, &epev)) {
        XCO_LOGFATAL(XCO_EXP_FUNC_WITH_ARG(fd, op, ev));
        XCO_ASSERT(false);
        return 0;
    }
...
uint32_t IoManager::TrgEvent(int fd, uint32_t evs) {
    ...
    epev.data.ptr = &ctx;
    if (epoll_ctl(epoll_fd_, op, fd, &epev)) {
        XCO_LOGFATAL(XCO_EXP_FUNC_WITH_ARG(fd, op, evs, change_evs, remain_evs));
        XCO_ASSERT(false);
        return 0;
    }
...
uint32_t IoManager::DelEvent(int fd, uint32_t evs) {
    ...
    epev.data.ptr = &ctx;
    if (epoll_ctl(epoll_fd_, op, fd, &epev)) {
        XCO_LOGFATAL("DelEvent" << XCO_EXP_FUNC_WITH_ARG(fd, op, evs, change_evs, remain_evs));
        XCO_ASSERT(false);
        return 0;
    }
...
void IoManager::OnIdle() {
    ...
    while(true) {
        ...
        XCO_LOGDEBUG("epoll_wait, " << XCO_EXP_VARS(ret))
